...
void scanToMapRegi(PointCloudPtr scan_points, KD_TREE<PointXYZ> &kdtree_map,
    ...
    for (size_t j = 0; j < scan2map_iteration; j++) {
        ...
        // cout << scan2map_iteration << "th iteration, rotation change:" << gtsam::Rot3::Logmap(delta.rotation()).norm() * 57.3 << endl;
        // cout << scan2map_iteration << "th iteration, translation change:" << delta.translation().norm() * 100 << endl;
        if (gtsam::Rot3::Logmap(delta.rotation()).norm() * 57.3 < 0.05 &&
            delta.translation().norm() * 100 < 1) // 0.05deg and 1cm
            break;
    }
}

void scanToMapRegi(pcl::PointCloud<pcl::PointXYZINormal>::Ptr scan_points, KD_TREE<pcl::PointXYZINormal> &kdtree_map,
                   gtsam::Pose3 &scan_pose,
                   int scan2map_iteration, double reject_distance,
                   double huber_threshhold, double plane_distance)
{
    vector<pcl::PointXYZINormal, Eigen::aligned_allocator<pcl::PointXYZINormal>> nearest_points;
    vector<float> point_search_sqdis;
    ...
    for (size_t j = 0; j < scan2map_iteration; j++) {
        ...
        for (size_t k = 0; k < scan_points->size(); k++) {
            pcl::PointXYZINormal point_ori = scan_points->points[k];
            pcl::PointXYZINormal point_in_map;
            ...
            if (point_search_sqdis[4] < reject_distance * reject_distance) {
                ...
                for (int k1 = 0; k1 < 5; ++k1) {
                    ...
                    if (fabs(norm(0) * nearest_points[k1].x +
                             norm(1) * nearest_points[k1].y +
                             norm(2) * nearest_points[k1].z + d) > plane_distance) {
                        plane_valid = false;
                        break;
                    }
                }
                ...
                if (plane_valid) {
                    ...
                    gtsam::noiseModel::Robust::shared_ptr surf_noise_model = gtsam::noiseModel::Robust::Create(
                            gtsam::noiseModel::mEstimator::Huber::Create(huber_threshhold), surf_gaussian_model);
...
void scanToMapRegi(PointCloudPtr scan_points, PointCloudPtr map,
                   ...
                   int scan2map_iteration, double reject_distance, double huber_threshhold, double plane_distance)
{
    ...
    for (size_t j = 0; j < scan2map_iteration; j++)
    {
            ...
            if (pointSearchSqDis[4] < reject_distance * reject_distance)
            {
                ...
                for (int k1 = 0; k1 < 5; ++k1)
                {
                    ...
                    if (fabs(norm(0) * map->points[pointSearchInd[k1]].x +
                             ...
                             norm(2) * map->points[pointSearchInd[k1]].z + d) > plane_distance)
                    {
                        plane_valid = false;
                        break;
                    }
        ...
        // cout << scan2map_iteration << "th iteration, rotation change:" << gtsam::Rot3::Logmap(delta.rotation()).norm() * 57.3 << endl;
        // cout << scan2map_iteration << "th iteration, translation change:" << delta.translation().norm() * 100 << endl;
        if (gtsam::Rot3::Logmap(delta.rotation()).norm() * 57.3 < 0.05 && delta.translation().norm() * 100 < 1) // 0.05deg and 1cm
            break;
    }
}

// PCL ICP with point to plane
bool Point2PlaneICP(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &source_cloud_normal,
                    const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &target_cloud_normal,
                    double distance_threshold, pcl::PointCloud<pcl::PointXYZINormal> &transformed_source,
                    double &fitness_score, Eigen::Matrix4d &transformation_src2tgt) {
    if (!source_cloud_normal || !target_cloud_normal) {
        return false;
    }
    if (source_cloud_normal->empty() || target_cloud_normal->empty()) {
        return false;
    }
    pcl::IterativeClosestPointWithNormals<pcl::PointXYZINormal, pcl::PointXYZINormal,double> icp_normal;
    icp_normal.setInputSource(source_cloud_normal);
    icp_normal.setInputTarget(target_cloud_normal);
    icp_normal.setTransformationEpsilon(1.e-10);
    icp_normal.setMaxCorrespondenceDistance(distance_threshold);
    icp_normal.setEuclideanFitnessEpsilon(0.001);
    icp_normal.setMaximumIterations(50);
    icp_normal.align(transformed_source, transformation_src2tgt);
    if (!icp_normal.hasConverged()) {
        cout << "ICP doesn't converge";
        return false;
    }
    fitness_score = icp_normal.getFitnessScore();
    transformation_src2tgt = icp_normal.getFinalTransformation().cast<double>();
    return true;
}

// Compute and Concatenate normal of point cloud
void ComputeAndConcatenateNormal(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud,
                                 pcl::PointCloud<pcl::PointXYZINormal>::Ptr &cloud_with_normals,
                                 double search_radius) {
    if (!cloud) {
        return;
    }
    pcl::NormalEstimationOMP<pcl::PointXYZ, pcl::PointNormal> ne; // normal estimation with openmp
    ...
    for (int i = 0; i < cloud->points.size(); ++i) {
        ...
        pt_normal.y = pt.y;
        pt_normal.z = pt.z;
        pt_normal.normal_x = normal.normal_x;
        pt_normal.normal_y = normal.normal_y;
        pt_normal.normal_z = normal.normal_z;
        if (!std::isnormal(pt_normal.normal_x) ||
            !std::isnormal(pt_normal.normal_y) ||
            !std::isnormal(pt_normal.normal_z)) {
            continue;
        }
        cloud_with_normals->push_back(pt_normal);
    }
}
