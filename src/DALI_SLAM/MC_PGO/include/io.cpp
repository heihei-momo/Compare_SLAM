...
bool compare_time_func1 (ouster_ros::Point pt1,ouster_ros::Point pt2) { return (pt1.t < pt2.t); }
bool compare_time_func2 (velodyne_ros::Point pt1,velodyne_ros::Point pt2) { return (pt1.time < pt2.time); }
...
void parseOusterLidar(const sensor_msgs::PointCloud2::ConstPtr &msg, int point_filter_num, LidarData &one_frame_lidar_data)
{
    ...
    sort (pl_orig.points.begin(), pl_orig.points.end(),compare_time_func1);
    ...
    for (int i = 0; i < point_num; i += point_filter_num)
    {
        if ((!std::isnan(pl_orig.points[i].x)) && (!std::isnan(pl_orig.points[i].y)) && (!std::isnan(pl_orig.points[i].z)))
        {
            ...
            pt.y = pl_orig.points[i].y;
            pt.z = pl_orig.points[i].z;
            pt.relative_time_ms = pl_orig.points[i].t / 1000000.0;
            //std::cout << pt.relative_time_ms << " ";
            pt.reflectivity = pl_orig.points[i].intensity;
            double dist = std::sqrt(pt.x * pt.x + pt.y * pt.y + pt.z * pt.z);
            if (dist > 100.0)
...
void parseVlp16Lidar(const sensor_msgs::PointCloud2::ConstPtr &msg, int point_filter_num, LidarData &one_frame_lidar_data)
{
    ...
    double omega_l = 0.361 * 10; // scan angular velocity 每1ms转的角度
    ...
    std::vector<float> time_last(16, 0.0);  // last offset time
    if (pl_orig.points[point_num - 1].time > 0)
    {
        given_offset_time = true;
    }
    //Todo hard code for simulation
    //given_offset_time = true;
    ...
    for (int i = 0; i < point_num; i += point_filter_num)
    {
        if ((!std::isnan(pl_orig.points[i].x)) && (!std::isnan(pl_orig.points[i].y)) && (!std::isnan(pl_orig.points[i].z)))
        {
            ...
            if (given_offset_time)
                pt.relative_time_ms = pl_orig.points[i].time / 1000.0;
            else{
                double yaw_angle = atan2(pt.y, pt.x) * 57.2957;
                if (is_first[layer]) {
                    // printf("layer: %d; is first: %d", layer, is_first[layer]);
                    yaw_fp[layer] = yaw_angle;
                    is_first[layer] = false;
                    pt.relative_time_ms = 0.0;
                    yaw_last[layer] = yaw_angle;
                    time_last[layer] = pt.relative_time_ms;
                    continue;
                }

                if (yaw_angle <= yaw_fp[layer]) {
                    pt.relative_time_ms = (yaw_fp[layer] - yaw_angle) / omega_l;
                } else {
                    pt.relative_time_ms = (yaw_fp[layer] - yaw_angle + 360.0) / omega_l;
                }

                if (pt.relative_time_ms < time_last[layer])
                    pt.relative_time_ms += 360.0 / omega_l;

                yaw_last[layer] = yaw_angle;
                time_last[layer] = pt.relative_time_ms;
            }
            //std::cout << pt.relative_time_ms << " ";
            //std::cout <<  pl_orig.points[i].ring << " ";
            pt.reflectivity = pl_orig.points[i].intensity;
            double dist = std::sqrt(pt.x * pt.x + pt.y * pt.y + pt.z * pt.z);
            if (dist > 100.0)
            {
                             ...
                             std::vector<LidarData> &lidar_data)
{
    ...
    for (auto &m : rosbag::View(bag, rosbag::TopicQuery(topics), start, end))
    {
        ...
        switch (lidar_type)
        {
            ...
            case VELO16:
                parseVlp16Lidar(m.instantiate<sensor_msgs::PointCloud2>(), point_filter_num, one_frame_lidar_data);
                lidar_data.push_back(one_frame_lidar_data);
                break;

            case OUST64:
                parseOusterLidar(m.instantiate<sensor_msgs::PointCloud2>(), point_filter_num, one_frame_lidar_data);
                lidar_data.push_back(one_frame_lidar_data);
                break;
...
void loadImuDataFromRosbag(std::string rosbag_path, std::vector<ImuData> &imu_data, double &dt)
{
    ...
    //cout << "dt: " << dt << endl;
    //cout << "imu_data size: " << imu_data.size() << endl;
    dt /= imu_data.size()-1;
...
void loadExtrinsic(std::string path, gtsam::Pose3 &extrinsic)
{
    ifstream ifs(path);
    if (!ifs) {
        cout << "extrinsic file fail to read" << endl;
        return;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        ifs >> a >> b >> c >> d;
        // 存储数据到矩阵
        matrix.row(i) << a, b, c, d;
    }

    //cout << matrix << endl;
    extrinsic = gtsam::Pose3(gtsam::Rot3(matrix.block<3,3>(0,0)),gtsam::Point3(matrix.block<3,1>(0,3)));
    cout << "li_extrinsic: " << endl;
    extrinsic.print();
