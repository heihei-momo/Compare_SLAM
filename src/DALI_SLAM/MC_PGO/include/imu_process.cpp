...
void rotation_integration (const std::vector<ImuData> &data, double imu_dt, std::vector<PoseData> &vec_rotation)
{
    auto w0 = data[0].wm;
    gtsam::Rot3 rotation;
    PoseData one_imu_rotation;
    one_imu_rotation.pose = gtsam::Pose3(rotation,gtsam::Point3());
    one_imu_rotation.timestamp = data[0].timestamp_s;
    vec_rotation.push_back(one_imu_rotation);
    for (int i = 1; i < data.size(); i++) {
        auto w1 = data[i].wm;
        auto w_avg = 0.5 * (w0 + w1);
        auto rotation_vector = w_avg * imu_dt;
        ...
        PoseData one_imu_rotation;
        one_imu_rotation.pose = gtsam::Pose3(rotation,gtsam::Point3());
        one_imu_rotation.timestamp = data[i].timestamp_s;
        vec_rotation.push_back(one_imu_rotation);
        w0 = w1;
    }
}