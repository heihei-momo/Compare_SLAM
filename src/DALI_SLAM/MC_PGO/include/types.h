...
struct PointXYZIT {
    ...
    double relative_time_ms = 0;// relative time with frame start time
};
...
struct Submap {
    ...
    ITPointCloud ITpointcloud_dt; // pt (with intensity and time) in submap_start_time coordinate using odom_poses and imu integration 
