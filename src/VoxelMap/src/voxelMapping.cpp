...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("robotpose/csv_file", robot_pose_csv_file, std::string(ROOT_DIR) + "Robotpose/robot_pose.csv");
  std::cout << "[VoxelMap] 输入点云距离截断: min(blind)=" << p_pre->blind
            << " m, max_range=" << p_pre->max_range << " m (<=0 表示不限制)" << std::endl;
  ...
  while (status) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (robot_pose_csv_en) {
        ...
        rec.px = state.pos_end(0);
        rec.py = state.pos_end(1);
        rec.pz = state.pos_end(2);
        ...
        rec.roll  = atan2(sinr, cosr) * 180.0 / M_PI;
        rec.pitch = asin(sinp > 1.0 ? 1.0 : (sinp < -1.0 ? -1.0 : sinp)) * 180.0 / M_PI;
        rec.yaw   = atan2(siny, cosy) * 180.0 / M_PI;
  ...
  if (robot_pose_csv_en) {
    ...
    if (csv_file.size() < 4 || csv_file.compare(csv_file.size() - 4, 4, ".csv") != 0) {
      if (!csv_file.empty() && csv_file[csv_file.size() - 1] != '/') csv_file += "/";
      csv_file += "robot_pose.csv";
    }
    // csv 路径支持相对写法: 相对路径以本方法源码目录(src/VoxelMap/)为基准, 绝对路径原样使用
    if (!csv_file.empty() && csv_file[0] != '/') csv_file = string(ROOT_DIR) + csv_file;
    const string csv_directory = csv_file.substr(0, csv_file.find_last_of('/'));
    ...
    if (csv_out.is_open()) {
      ...
      csv_out.close();
      cout << "[VoxelMap] robot pose csv saved to: " << csv_file
           << " (" << robot_pose_records.size() << " poses)" << endl;
    } else {
      cout << "[VoxelMap] fail to create csv file: " << csv_file << endl;
    }
  }
  ...
  cout << "\n****************************************************" << endl;
  if (frame_num > 0) {
    cout << "[VoxelMap] 平均单帧处理耗时: " << fixed << setprecision(2) << aver_time_consu
         << " ms  |  处理帧率(FPS): " << 1000.0 / aver_time_consu
         << "  (总帧数 " << frame_num << ")" << endl;
    cout << "[VoxelMap] 有效观测点数量: 平均 " << mean_effect_points << " 点/帧" << endl;
    cout << "[VoxelMap] 原始点/下采样点: 平均 " << mean_raw_points << " / "
         << mean_ds_points << " 点/帧" << endl;
  } else {
    cout << "[VoxelMap] 没有处理任何帧(未产生轨迹)" << endl;
  }
  cout << "****************************************************" << endl;
