        ...
        if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) ||
            ...
            (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7) &&
                (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y +
                 ...
                 blind * blind)) {
          // 距离截断: 太远的点不标记为有效
          const double range_sq = pl_full[i].x * pl_full[i].x +
...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  for (int i = 0; i < pl_orig.points.size(); i++) {
    ...
    if (range < blind)
      continue;
    if (max_range > 0.0 && range > max_range * max_range)
      continue;   // 距离截断: 去掉太远(max_range)的点
...
void Preprocess::l515_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  for (int i = 0; i < pl_orig.size(); i++) {
    ...
    if (i % point_filter_num == 0) {
      // 距离截断(该分支原本没有距离过滤)
      const double range_sq = added_pt.x * added_pt.x + added_pt.y * added_pt.y +
                              added_pt.z * added_pt.z;
      if (range_sq < blind * blind) continue;
      if (max_range > 0.0 && range_sq > max_range * max_range) continue;
...
void Preprocess::velodyne_handler(
  ...
  for (int i = 0; i < pl_orig.size(); i++) {
    ...
    // 距离截断(原版 velodyne 分支没有任何距离过滤, 这里补上最小/最大距离)
    const double range_sq = added_pt.x * added_pt.x + added_pt.y * added_pt.y +
                            added_pt.z * added_pt.z;
    if (range_sq < blind * blind) continue;                              // 太近(盲区)
    if (max_range > 0.0 && range_sq > max_range * max_range) continue;   // 太远
